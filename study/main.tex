...
This report presents a deep learning framework for detecting SYN flood
\gls{ddos} attacks directly from raw network traffic. Packet streams are
encoded as grayscale images using a \textbf{0.3\,s time-window} aggregation
strategy. The model is trained and evaluated on data derived from the public
CICDDoS2019 dataset. The classifier is a
ResNeXt-50 (32$\times$4d) \gls{cnn}, implemented in Huawei MindSpore with
\mbox{\texttt{mindcv}}, and optimized for binary classification (DDoS vs.\ benign)
using focal loss and DDoS-prioritized early stopping.

In same-domain offline testing (2026-04-03), the system achieves
\textbf{98.1\%} accuracy and a macro F1-score of \textbf{0.937}. A companion
CPU-based live inference pipeline provides real-time, window-level
predictions. However, these online outputs should not be interpreted as direct
equivalents of offline test performance in the absence of labeled deployment
traffic for calibration.
...
imbalance-aware training variants in code.
...
systematic comparison of time-window lengths and dual-view (all-packets vs.\
SYN-only) fusion; hardware acceleration beyond CPU-only deployment;
stress-testing under concept drift, adversarial perturbations, and
\textbf{domain adaptation} when live packet-to-image rules differ from offline
windows; and \textbf{calibration} of live alerts (thresholds on
