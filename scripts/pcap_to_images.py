...
def packet_to_image_224x224(pkt_bytes, syn_only):
    """
    Per paper (Section III.B): convert packet to 2D grayscale image, then resize to 224x224.
    """
    ...
    img_2d = arr.reshape((n, n))

    # Resize to 224x224 as in the paper (PIL)
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert PCAP packets to 224x224 grayscale images (paper methodology, Section III.B)."
    )
    ...
    parser.add_argument(
        "--syn-only",
        action="store_true",
        help="Only convert TCP SYN packets (for DDoS traffic).",
    )
    ...
    for pkt_bytes in iter_pcap_packets(pcap_path):
        img_arr = packet_to_image_224x224(pkt_bytes, args.syn_only)
        if img_arr is None:
            continue
